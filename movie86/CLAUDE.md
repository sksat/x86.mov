...
- **`AbiHost` is the `int`-less mov-only counterpart** of `SysHost` / `LibcHost` / `BiosHost` — guest writes to the unmapped page at `ABI_BASE = 0x1FFE_0000` dispatch through `AbiHost::abi_call(call_num, value, regs, mem)`. New host calls pick an unused offset on the page (see `core/src/abi_host.rs::CALL_*`), wire it on the matching turbo86 side **in the same PR** (the constants are a cross-engine contract), and add a Cpu unit test that pins the dispatch with a hand-built `mov [ABI_BASE+0xNN], al/eax` fixture. The default trap (`Fault::UnsupportedAbiCall`) is the right behaviour for hosts that don't recognise a call — same shape as the `BiosHost` default.
