...
- **`int`-less mov-only ABI ([`AbiHost`](core/src/abi_host.rs)).** Guests trigger host calls by `mov`-ing into the otherwise-unmapped page at [`ABI_BASE = 0x1FFE_0000`](core/src/abi_host.rs); `Cpu::step`'s mov path gates writes whose computed destination falls in that page and routes them to [`AbiHost::abi_call(call_num, value, regs, mem)`](core/src/abi_host.rs) instead of `Memory::write_u8`/`u32`. Call numbers are page-offset constants ([`CALL_SET_VIDEO_MODE = 0x010`](core/src/abi_host.rs), [`CALL_MMAP_REQUEST = 0x020`](core/src/abi_host.rs), [`CALL_WRITE = 0x080`](core/src/abi_host.rs), [`CALL_EXIT = 0x0FE`](core/src/abi_host.rs)); `regs` is a `&mut [u32; 8]` so `CALL_WRITE` can read `ebx`/`ecx`/`edx` for fd/buf/len and write `eax` for bytes-written. The contract is byte-for-byte mirrored on the turbo86 side (see [turbo86/DESIGN.md §Mov-only ABI page](../turbo86/DESIGN.md)) so the same canvas ELF runs natively under both engines without a per-engine binding step.

  - **`int 0x10` / `int 0x80` are kept for legacy fixtures only.** `WasmHost::bios_call` still handles `int 0x10 / AH=0` for the older committed canvas binaries (`canvas_smile.elf`, `canvas_modes.elf`) whose sources we don't have; `int 0x80 SYS_write`/`SYS_exit` remain implemented for the same reason. New examples and stubs use the mov-only ABI exclusively — see [`movie86/wasm/examples/sources/`](wasm/examples/sources/) for the rebuilt stubs and [`#28`](https://github.com/sksat/x86.mov/issues/28) for the migration narrative.
  - **`Fault::UnsupportedAbiCall(call_num)`** is the trait's default trap so a typo in a guest's ABI use surfaces loudly — same shape as `Fault::UnsupportedInterrupt`. Hosts that don't care about the ABI write `impl AbiHost for MyHost {}` and inherit the trap.
