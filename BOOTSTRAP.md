...
> 2. **「その先に待っているメモリの壁」は無かった。** 一度はそう書いたが、
>    ~27 GiB というのはピークではなく、停止しない無限ループが機械の RAM を
>    食い切っていただけだった（原因は `setTruncStoreAction` 1 行）。
>    直した結果と、その先で見つかった narrow store の健全性バグを潰した結果、
>    **lcc の 32 翻訳単位は 32/32 が mov-only にコンパイルでき、全体で 4 秒**である。
...
| + i16 / i1 ext-load | 21 | 1 | 10 |
| + GAS 予約語シンボル | 22 | 0 | 10 |
| + i16 トラックストアの無限ループ | **22** | **0** | 10 |

**通る本数は変わらないが、無限ループが 0 になった。** 残る 10 本は
1〜2 秒で `Cannot select: store` と落ちる — i16 ストアを意図的に拒否している
ためで、理由は [`llvm-mov/GAP-MATRIX.md`](llvm-mov/GAP-MATRIX.md) §3.2b
(ワード RMW 方式は同一ワードへの独立した narrow store 2 本で壊れる)。
**コーパス全体が数秒で決着し、二度と機械を殺さない。**

以下は「残る 10 本は資源の問題」と考えていた頃の測定で、記録として残す。
実際にはこれらは OOM ではなく無限ループで、上の 2 行の修正で全て通るようになった:
...
x86linux          200   25.3 GiB  (OOM kill)
```

この「26 GiB で OOM」という読み自体が誤りだった。ピークは与えたメモリ量に
張り付くだけで、有限の要求ではない。`/usr/bin/time -v` は SIGKILL された
プロセスにも `Exit status: 0` と印字するので、`dag.c` が「104 秒 / 27.4 GiB で
成功した」という観測も成立していなかった。

**機能上の壁も資源の壁も無くなった。** 現在 32/32 が 4 秒で通る。
...
**対処済み**: テーブルを COMDAT + weak にした。リンクの仕方を変えずに、
リンカが 1 部だけ残す。4 オブジェクトのリンクで `.rodata` 3,428,352 → 857,088 B、
lcc 32 オブジェクトで 55 MB → 30 MB。`make test-link-dedup` が見る。
...
lcc の 32 翻訳単位のうち 10 本がこの領域にいて、29 GiB のマシンでは
...
**L7（実 Linux に `arch/x86mov32` を追加して boot）を塞いでいた前提は両方とも消えた** —
テーブル複製（§5.1）もピークメモリ（§5.2）も解決済みである。
残るのは linux-mov 側の実装そのもの（L0 → L1 → …）になった。
...
1. ~~ピークメモリ~~ — **完了**。ピークではなく無限ループだった（§5.2）
1b. ~~narrow store を本物のバイトストアに~~ — **完了**。`STORE8` 疑似命令 +
   post-RA 展開。健全性が直り、副産物として `.text` が 13〜35 % 縮んだ
2. ~~バイトテーブルを共有ランタイムへ~~ — **完了**。COMDAT + weak で、
   リンク時に 1 部だけ残る（lcc 32 オブジェクトで 55 MB → 30 MB）

**P0 は空になった。** 次は P2 のゴール別作業に進める。
