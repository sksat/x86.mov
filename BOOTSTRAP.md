...
>    食い切っていただけだった（原因は `setTruncStoreAction` 1 行）。直した結果、
>    **lcc の 32 翻訳単位は 32/32 が mov-only にコンパイルでき、全体で 10 秒**である。
...
| + GAS 予約語シンボル | 22 | 0 | 10 |
| + i16 トラックストアの無限ループ | 31 | 0 | 1 |
| + アライン不足 i16 ロード | **32** | **0** | **0** |

**32/32 が通り、全体の所要時間は 10 秒である。** 最大の翻訳単位
(`x86linux.c`、IR 18,826 行) で 4 秒。
