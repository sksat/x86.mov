...
>    直した結果、**ハングは 0 になり、コーパス全体が数秒で決着する**。
>    通る本数は 22/32 のままで、残りは i16 ストアの健全な lowering 待ちである。
...
**機能上の壁も資源の壁も無くなった。** 現在 32/32 が 10 秒で通る。
...
一方で、**L7（実 Linux に `arch/x86mov32` を追加して boot）は §5.1 の解決が前提**。
数千 TU × 860 KiB のテーブル複製ではイメージが成立しない。
（メモリのほうは §5.2 のとおり解決済みで、もう前提ではない。）
...
1b. **narrow store を本物のバイトストアにする** — 現在のワード RMW 方式は
   同一ワードへの独立した narrow store 2 本で壊れるため i16 を拒否している。
   `MOV8mr` は post-RA 専用なので、ISel にバイトストア経路を与える作業
2. **バイトテーブルを共有ランタイムへ**（COMDAT + weak、または `libmovrt.a` 切り出し）
   — 唯一残った P0。rcc 32 本のオブジェクト合計 70 MB に対し `.text` は 32.2 MiB で、
   差の約 27 MB がテーブルのコピーである
