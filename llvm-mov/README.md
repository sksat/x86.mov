...
| 6f     | **What real C needs.** Measured by compiling all 32 translation units of lcc + the M/o/Vfuscator backend ([`GAP-MATRIX.md`](GAP-MATRIX.md)) — the corpus furthest from this backend's Rust fixtures. Four things landed: i386 SysV **varargs**; **`fastcc`** (clang promotes `internal` functions to it at -O1, so any optimised C hits it); **i16 / i1 ext-loads** (i8's Custom path, width-parameterised); two **`SELECT` infinite loops** — what had been recorded as "compile time is super-linear on some functions" turned out to be the legalizer cycling `SELECT → SELECT_CC → SELECT` with flat memory, once via pointer-typed selects and once via SELECTs the legalizer synthesises itself; and **symbols named after GAS Intel-syntax reserved words** (`offset`, `and`, …), which could not be referenced at all. Compiling rcc went from 7/32 translation units to 22/32; what stops the remaining 10 is peak memory (~27 GiB for one translation unit), not a missing feature. | exec: `vararg_sum`, `fastcc_call`, `ptr_select`, `loop_diamond_bound`, `narrow_load`; MovOnly: `vararg_sum` | ✅ |
