...
| 7h2    | f64 fcmp via SDAG soft-float → `__{eq,ne,lt,le,gt,ge,unord}df2` injected as IR. Helper body mirrors 7g1 f32 fcmp but operates over {hi,lo} i32 pairs (no i64 select, no i64 compare — just constant-amount `lshr i64 X, 32` for the split). | exec: `f64_fcmp_{eq,eq_false,lt,lt_false,neg,zero,nan_oeq,nan_uno,ge}`; MovOnly: `f64_fcmp` | ✅ |
| 7h3    | i32 ↔ f64 conversions via `__floatsidf` / `__floatunsidf` (i32→f64, always-exact since 32 < 53) and `__fixdfsi` / `__fixunsdfsi` (f64→i32 truncate-toward-zero with saturation). First helpers in the codebase to use variable-amount i32 shifts to emulate variable-amount i64 shifts via clamped-arm i32-pair split. | exec: `f64_sitofp_{rt,rt_neg,rt_zero,rt_large,exp}`, `f64_uitofp_rt`, `f64_fptosi_{truncate,neg,zero}`; MovOnly: `f64_sitofp`, `f64_fptosi` | ✅ |
| 7h4    | f64 fadd / fsub via SDAG soft-float → `__adddf3` / `__subdf3` injected as IR. Mirrors 7g1 f32 `__addsf3` widened to 53-bit mantissa with 3-bit guard region; variable-amount i64 shifts emulated via 7h3's clamped-arm i32-pair split (`emitLshrI64ByI32` / `emitShlI64ByI32`); i64 compares manually expanded to {hi, lo} i32-pair compares to avoid DAG-ISel multi-minute pathology with i64 SETCC. Inf / NaN propagation per 7g4. | exec: `f64_fadd_{rt,rt_neg,rt_zero,rt_cancel,rt_big,nan,inf}`, `f64_fsub_{rt,rt_neg}`; MovOnly: `f64_fadd`, `f64_fsub` | ✅ |
| 7h5    | f64 fmul via SDAG soft-float → `__muldf3` injected as IR. Mantissa multiply = 53×53 → 106-bit product computed via four 32×32→64-bit sub-multiplies (each split internally into four 16×16 `mul i32`, lowered by stage-7f1 byte-table); product carried as {High64, Low64} i64 pair; bit-105 / bit-104 normalize cases with round-to-nearest-ties-to-even. Inf / NaN propagation per 7g4. | exec: `f64_fmul_{rt,rt_neg,rt_zero,rt_one,rt_big,nan,zero_inf,inf_finite}`; MovOnly: `f64_fmul` | ✅ |
| 7h6    | f64 fdiv via SDAG soft-float → `__divdf3` injected as IR. 52-iter restoring long-division loop with real CondBr / PHI control flow, mirroring stage-7g3 `__divsf3`'s shape widened to i64. Loop-body `select i64`s get bit-blended (helper-safe attr); i64 compares use the 7h4 `UgeI64` / `IsNonZeroI64` lambdas. Stage-7g4-style Inf / NaN / zero propagation: 0/0, Inf/Inf, NaN inputs → qNaN; Inf/finite → signed Inf; finite/Inf → signed zero. | exec: `f64_fdiv_{rt,rt_neg,rt_one,rt_renorm,rt_zero_over,by_zero,nan,inf_inf,zero_zero}`; MovOnly: `f64_fdiv` | ✅ |
| 7h7    | First end-to-end f64 Rust demo (`examples/rust/mandelbrot`): sum of mandelbrot escape-iter counts along the real axis (16 evenly-spaced points). Exercises the 7g / 7h helper surface through plain `cargo build` — `sitofp` / `fmul` / `fadd` / `fsub` / `fcmp` (olt / ole / ogt) all route to injected libcalls; 337 mod 256 → exit 81 matching x86-64 native bit-for-bit. | `test-rust-example` gate's `mandelbrot` slot | ✅ |
| 6e     | indirect `call` via function pointer (CALL32r)     | `indirect_call` exec + MovOnly                           | ✅    |
| 6f     | **What real C needs.** Measured by compiling all 32 translation units of lcc + the M/o/Vfuscator backend ([`GAP-MATRIX.md`](GAP-MATRIX.md)) — the corpus furthest from this backend's Rust fixtures. Five things landed: i386 SysV **varargs**; **`fastcc`** (clang promotes `internal` functions to it at -O1, so any optimised C hits it); **i16 / i1 ext-loads** (i8's Custom path, width-parameterised); two **`SELECT` infinite loops** — what had been recorded as "compile time is super-linear on some functions" turned out to be the legalizer cycling `SELECT → SELECT_CC → SELECT` with flat memory, once via pointer-typed selects and once via SELECTs the legalizer synthesises itself; and **symbols named after GAS Intel-syntax reserved words** (`offset`, `and`, …), which could not be referenced at all. Compiling rcc went from 7/32 translation units to 22/32; what stops the remaining 10 is peak memory (~27 GiB for one translation unit), not a missing feature. | exec: `vararg_sum`, `fastcc_call`, `ptr_select`, `loop_diamond_bound`, `narrow_load`, `gas_keyword_symbol`; MovOnly: `vararg_sum` | ✅ |
