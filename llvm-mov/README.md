...
| 6f     | i386 SysV varargs (`va_start` + vararg calls) and `fastcc`. Both are things *real C* needs the moment it is compiled with optimisation — clang promotes `internal` functions to `fastcc` at -O1, and 5 of lcc's translation units define vararg functions. See [`GAP-MATRIX.md`](GAP-MATRIX.md) for the measured survey that motivated this. | exec: `vararg_sum`, `fastcc_call`; MovOnly: `vararg_sum` | ✅ |
