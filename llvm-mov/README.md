...
| 6f     | **What real C needs.** Measured by compiling all 32 translation units of lcc + the M/o/Vfuscator backend ([`GAP-MATRIX.md`](GAP-MATRIX.md)) — the corpus furthest from this backend's Rust fixtures. Four things landed: i386 SysV **varargs**; **`fastcc`** (clang promotes `internal` functions to it at -O1, so any optimised C hits it); **i16 / i1 ext-loads** (i8's Custom path, width-parameterised); and two **`SELECT` infinite loops** — what had been recorded as "compile time is super-linear on some functions" turned out to be the legalizer cycling `SELECT → SELECT_CC → SELECT` with flat memory, once via pointer-typed selects and once via SELECTs the legalizer synthesises itself. Compiling rcc went from 7/32 translation units to 20/32. | exec: `vararg_sum`, `fastcc_call`, `ptr_select`, `loop_diamond_bound`, `narrow_load`; MovOnly: `vararg_sum` | ✅ |
