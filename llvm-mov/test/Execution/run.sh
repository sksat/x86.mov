...
# Sanity-check the toolchain up front — way easier to read than a `make`
# error 200 lines down.
for tool in "$DRIVER" as ld; do
    if ! [ -x "$DRIVER" ] && [ "$tool" = "$DRIVER" ]; then
...
for ll in "${HERE}"/*.ll; do
    ...
    if ! ld -m elf_i386 -static -e _start -o "${elf}" "${WORK}/_start.o" "${o}" 2>"${WORK}/${name}.ld.log"; then
