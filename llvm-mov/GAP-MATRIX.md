...
**対処済み**: テーブルを COMDAT グループに入れ、シンボルを weak にした。
「複数のオブジェクトが同じテーブルを定義している」が重複定義エラーではなく
正常な状態になり、リンカが 1 部だけ残す。

```
4 オブジェクトのリンク:   .rodata 3,428,352 B → 857,088 B
lcc 32 オブジェクト:      55 MB → 30 MB（テーブル 837 KiB = 1 部）
```

ランタイムアーカイブ（`libmovrt.a` 相当）への切り出しではなく COMDAT を
選んだのは、リンクの仕方を一切変えなくてよいからである — テストランナーも
`cargo-link.sh` も bench も examples も既存のリンク行のまま効く。
テーブルごとのセクション分けも残るので `--gc-sections` の効きも失わない。

`make test-link-dedup` がこれを見る。既存のゲートはどれも 1 オブジェクトしか
リンクしないので、この退行は誰も見ていなかった。

なお**オブジェクト単体のサイズは変わらない**（各々がテーブルを持つ）。
効くのはリンク時である。
