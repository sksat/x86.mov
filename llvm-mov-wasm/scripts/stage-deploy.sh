#!/usr/bin/env bash
# Assemble the static deploy tree under ../dist/llvm-mov-wasm/ ready for
# Cloudflare Pages. Same shape as ../movfuscator-wasm/scripts/stage-deploy.sh:
# the source tree layout matches the served URL layout so the wrapper's
# relative imports (./build/llvm-mov-llc.js) resolve identically in dev
# (make serve) and on the deployed host.
#
# Layout produced:
#   dist/
